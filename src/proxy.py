...
class proxy_rotator:
    ...
    def __init__(self, proxies):
        ...
        self.current_xtt = proxies[0]
        
    def next(self):
        if self.proxies[-1] == self.current:
            self.current = self.proxies[0]
            self.current_xtt = self.proxies[0]
            self.current_irt = 1
        else:
            self.current = self.proxies[self.current_irt]
            self.current_xtt = self.proxies[self.current_irt]
            self.current_irt += 1
...
def make(total_ips):
    ...
    return proxy_rotator([f"http://127.0.0.1:{9080 + current_port}" for current_port in range(total_ips)])
